...
import { CreditCard, Calendar, CheckCircle2, XCircle, Clock, RefreshCw } from 'lucide-react';
...
const Payments = () => {
  ...
  if (loading) {
    return (
      <div className="space-y-6 animate-pulse">
        ...
        <div className="bg-bg-card border border-main rounded-[2rem] h-96"></div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-bg-card border border-main rounded-[2rem] overflow-hidden shadow-xl">
        {payments.length === 0 ? (
          <div className="p-20 text-center">
            <div className="w-16 h-16 bg-main/5 rounded-full flex items-center justify-center mx-auto mb-6">
              <CreditCard className="h-8 w-8 text-muted opacity-20" />
            </div>
            <h3 className="text-xl font-black text-main mb-2">No payment history</h3>
            <p className="text-muted">Your payment records will appear here once you subscribe.</p>
          </div>
        ) : (
